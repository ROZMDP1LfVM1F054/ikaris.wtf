...
function newRemote(type, name, args, remote, function_info, blocked, src, returnValue)
	...
	schedule(function()
		...
		if blocked then
			logs[#logs].GenScript = "-- THIS REMOTE WAS PREVENTED FROM FIRING THE SERVER BY Lunar Spy\n\n"
				.. logs[#logs].GenScript
		end
	end)
	...
	if keyToString then
		if typeof(v) == "userdata" and getrawmetatable(v) then
			return string.format(
				'"<void> (%s)" --[[Potentially hidden data (tostring in Lunar Spy:HookRemote/GetRemoteFiredSignal at your own risk)]]',
				safetostring(v)
			)
		elseif typeof(v) == "userdata" then
	...
	if not toggle then
		if hookmetamethod then
			...
			if not original then
				warn("Lunar Spy: namecall method not found!")
				onToggleButtonClick()
				return
			end
...
if not _G.SimpleSpyExecuted then
	local succeeded, err = pcall(function()
		if not RunService:IsClient() then
			error("Lunar Spy cannot run on the server!")
		end
		if
			not hookfunction
	...
	if not succeeded then
		warn(
			"A fatal error has occured, Lunar Spy was unable to launch properly.\nPlease DM this error message to @exx#9394:\n\n"
				.. tostring(err)
		)
		SimpleSpy2:Destroy()
		hookfunction(remoteEvent.FireServer, originalEvent)
...
newButton("Function Info", function()
	...
	if selected then
		...
		TextLabel.Text = "Done! Function info generated by the Lunar Spy Serializer."
	end
end)
...
newButton("CallingScript", function()
	return string.format(
		"[%s] [UNSAFE] Uses 'getcallingscript' to get calling script for Decompile and GetScript. Much more reliable, but opens up Lunar Spy to detection and/or instability.",
