...
local zuvware = Instance.new("ScreenGui")
zuvware.Name = "_zvw2"
zuvware.Parent = game.CoreGui
zuvware.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
local usi = game:GetService("UserInputService")
local uitoggled = false
usi.InputBegan:Connect(
	function(io, p)
		if io.KeyCode == Enum.KeyCode.BackSlash then
			if uitoggled == false then
				zuvware.Enabled = false
				uitoggled = true
			else
				zuvware.Enabled = true
				uitoggled = false
			end
		end
end)
...
function lib:notification(textt)
	for i, v in next, zuvware:GetChildren() do
	...
	notificationframe.Parent = zuvware
...
function lib:MainWindow(text)
	...
	function window:Tab(title)
		...
		function tab:Textbox(text, disapper, callback)
			...
			textboxmain.Text = "Username Here"
...
local s = lib:MainWindow("TEXT HERE")
local hm = s:Tab("Home")
local ss = s:Tab("Main")
...
ss:Textbox("EXAMPLE",true, function(value)

end)

-- COMBAT --

ss:Slider("Change Jumpower", 16, 1500, function(value)
